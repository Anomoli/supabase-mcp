...
LIVEKIT_URL = os.getenv(
    "LIVEKIT_PUBLIC_URL", os.getenv("LIVEKIT_URL", "ws://localhost:7880")
)
...
def main():
    ...
    print(f"token:\n{token}")
    print(
        "\nQuick test from a phone browser on the same Tailscale tailnet:\n"
        "  https://meet.livekit.io/custom?"
        f"liveKitUrl={quote(LIVEKIT_URL, safe='')}&token={quote(token, safe='')}\n"
        "(The Meet page is the client UI; LiveKit media terminates on Gammy.)"
    )
